"""
See github.com/NVIDIA/NeMo/blob/main/scripts/speech_recognition/convert_to_tarred_audio_dataset.py
Fixed syntax error to make it work.

Example usage:

python convert_to_tarred_audio_dataset.py \
    --manifest_path=/mnt/datadrive/datasets/packed_audio/train_manifest.jsonl \
    --target_dir=/mnt/datadrive/datasets/packed_audio/train/ \
    --num_shards=256 \
    --max_duration=28.0 \
    --min_duration=0.2 \
    --shuffle --shuffle_seed=11 \
    --keep_files_together --slice_with_offset \
    --workers=-1

python convert_to_tarred_audio_dataset.py \
    --manifest_path=/mnt/datadrive/datasets/packed_audio/dev_manifest.jsonl \
    --target_dir=/mnt/datadrive/datasets/packed_audio/dev/ \
    --num_shards=16 \
    --max_duration=28.0 \
    --min_duration=0.2 \
    --shuffle --shuffle_seed=11 \
    ...
    def create_concatenated_dataset(
        ...
        for new_manifest_idx in range(len(manifest_paths)):
            ...
            if len(filtered_new_entries) > 0:
                print(f"Filtered {len(filtered_new_entries)} files which amounts to {filtered_duration:0.2f} seconds of audio from manifest {manifest_paths[new_manifest_idx]}.")
        ...
        print(f"Dropping {drop_count} samples from total new samples {total_new_entries} since they cannot be added into a uniformly sized chunk.")
        ...
        with Parallel(n_jobs=num_workers, verbose=num_added_shards) as parallel:
            # Call parallel tarfile construction
            new_entries_list = parallel(
                delayed(self._create_shard)(entries[start_idx:end_idx], target_dir, shard_idx, manifest_folder, only_manifests)
        ...
        for entry in tqdm(entries, desc="Creating shard.."):
            ...
            if self.config.slice_with_offset:
                ...
                if len(entry_duration) == 1:
                    ...
                    raise ValueError(f"The duration for the entry with audio_filepath '{entry['audio_filepath']}' is incorrectly provided ({entry['duration']}).")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manifest_path", default=None, type=str, required=False, help="Path to the existing dataset's manifest.")
    ...
    parser.add_argument(
        "--dry_run",
        action="store_true",
        help=("Run in simulation mode: calculate and display the number of shards and estimated data per shard without reading audio files or writing any output."),
    )
